...
namespace Il2CppDumper
{
    public sealed class Elf : Il2Cpp
    {
        ...
        */
        ...
        public bool CheckSection(Stream stream)
        {
            try
            {
                ...
                {
                    var name = ReadStringToNull(shstrndx + section.sh_name);
                    names.Add(name);

                    if (name == ".text" || name == ".rodata")
                    {
                        int[] bytecnt = new int[256];
                        Position = section.sh_offset;
                        byte[] databytes = ReadBytes((int)section.sh_size);
                        foreach (var b in databytes)
                        {
                            bytecnt[b]++;
                        }
                        xorbyte = (byte)Array.IndexOf(bytecnt, bytecnt.Max());
                        if (xorbyte != 0x00)
                        {
                ...
                {
                    stream.Position = 0;
                    File.WriteAllBytes("libil2cpp.fixed.so", ReadBytes((int)stream.Length));
                }
